...
import { GET_ROL_INCIALIZAR,
    POST_ROL } from '../../../helpers/RolesMto/url_helper';
//HOOKS
import useApiHelper from '../../../hooks/useApiHelper';

const CrearRol = props =>{
    ...
    useEffect(()=>{
        if(modulos.length > 0) _formarModulosInicializados();
    },[modulos]);
    ...
    const _crearRol=async(values)=>{
        ...
        if(permisos.length > 0){
            ...
            const result = await post(POST_ROL, data);
            Swal.fire({
                title:"Crear Rol",
                icon:"success",
                text: result.message,
                                    ...
                                    <Row>
                                       {listaModulos.map((modulo, index)=>{
                                        if(modulo.recursos.length > 0){
                                            return(
                                                <Col lg={6} md={4} xs={12} sm={12} className='mr-2' key={index}>
                                                    <FormGroup>
